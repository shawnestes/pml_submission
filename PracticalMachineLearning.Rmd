...
Our dataset includes user activity levels from accelerometers worn during weight lifting exercises. The goal is to predict which activity was most efficient. 

### Data Sources

For detailed information on the dataset, see: http://groupware.les.inf.puc-rio.br/har">http://groupware.les.inf.puc-rio.br/har in particular the section on Weight Lifting Exercises. 
...
Random Forest is known for its accuracy, albeit not speed. With that in mind, this is a relatively small dataset so I've prioritized accuracy over speed. To greater reduce the speeds, I've chosen to subset the random forest to just the relevant columns. This is also helps to prevent levels issues in new datasets.
...
The random forest generated has a model that has an out-of-bag (OOB) error is 1.88%. I'd suspect the same results when predicting against the test set on this model. Let's test it and show the confusion matrix results. 
...
The accuracy rate of 98.1% matches closely with our OOB error estimates. Results were tested against the out of sample testing set successfully on all problems: 20 out of 20. 
